...
tela = Tk()
tela.title("Controle de Locais Turisticos - Cadastrando")
...
def limpar():
    ...
    txtValorEntrada.delete(0, tk.END)
...
def salvar():
    ...
    nome = txtNome.get()
    valorEntrada = txtValorEntrada.get()
    necessitaGuia = varRadioGuia.get()
    ...
    locais = {
        ...
        "necessitaGuia": necessitaGuia,
        "cidade": cidade,
        "uf": uf
    }

    collection.insert_one(locais)

    messagebox.showinfo("Mensagem", f"Local com o id: {codigo} foi cadastrado com sucesso.")
...
def update():
    ...
    messagebox.showinfo("Mensagem", f"Local com o id: {codigo} foi alterado com sucesso.")
...
def excluir():
    ...
    messagebox.showinfo("Mensagem", f"Local com o id: {codigo} foi deletado com sucesso.")
...
def consultar():
    ...
    if response:
        ...
        txtValorEntrada.insert(END, f"{response['valorEntrada']}")
        txtCidade.insert(END, f"{response['cidade']}")
        combo.insert(END, f"{response['uf']}")
        varRadioGuia.set(f"{response['necessitaGuia']}")

        messagebox.showinfo("Mensagem", f"Local com o id: {codigo} foi consultado com sucesso.")
    else:
...
lblValorEntrada = Label(tela, font="Arial 14", text="Valor da Entrada:", fg='#000', bg='#c0c0c0').place(x=250, y=120)
txtValorEntrada = Entry(tela, width=40, borderwidth=0, fg="black")
txtValorEntrada.place(x=410, y=127)

varRadioGuia = StringVar()
varRadioGuia.set("Sim")

radioButtonG = Radiobutton(tela, text="Sim", variable=varRadioGuia, value="Sim", bg='#c0c0c0').place(x=250, y=160)
radioButtonNG = Radiobutton(tela, text="Não", variable=varRadioGuia, value="Não", bg='#c0c0c0').place(x=300, y=160)

lblCidade = Label(tela, font="Arial 14", text="Cidade:", fg='#000', bg='#c0c0c0').place(x=250, y=200)
txtCidade = Entry(tela, width=40, borderwidth=0, fg="black")
txtCidade.place(x=330, y=207)

lblUf = Label(tela, font="Arial 14", text="Uf:", fg='#000', bg='#c0c0c0').place(x=250, y=240)
combo = ttk.Combobox(tela)
combo['values'] = ("São Paulo","Rio de Janeiro","Minas Gerais","Bahia","Ceará","Amazonas")
combo.current(2)
combo.place(x=290, y=247)
